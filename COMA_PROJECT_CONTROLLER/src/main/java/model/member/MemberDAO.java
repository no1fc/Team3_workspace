...
public class MemberDAO {
   ...
   private final String SERCHID = "SELECT MEMBER_ID,MEMBER_PASSWORD,MEMBER_NAME,MEMBER_PHONE,MEMBER_PROFILE,MEMBER_REGISTRATION_DATE,MEMBER_CURRENT_POINT,MEMBER_TOTAL_POINT,MEMBER_CREW_NUM,MEMBER_CREW_JOIN_DATE,MEMBER_LOCATION,MEMBER_ROLE\r\n"
         + "FROM MEMBER\r\n"
         + "WHERE MEMBER_ID = ?";
   
   //아이디 비밀번호로 찾기
   private final String SERCHIDPASS="SELECT MEMBER_ID,MEMBER_PASSWORD,MEMBER_NAME,MEMBER_PHONE,MEMBER_PROFILE,MEMBER_REGISTRATION_DATE,MEMBER_CURRENT_POINT,MEMBER_TOTAL_POINT,MEMBER_CREW_NUM,MEMBER_CREW_JOIN_DATE,MEMBER_LOCATION,MEMBER_ROLE\r\n"
         + "FROM MEMBER\r\n"
         + "WHERE MEMBER_ID = ? AND MEMBER_PASSWORD=?";
   
   //크루에 속한 회원목록 조회
   private final String SERCHCREW="SELECT MEMBER_ID,MEMBER_PASSWORD,MEMBER_NAME,MEMBER_PHONE,MEMBER_PROFILE,MEMBER_REGISTRATION_DATE,MEMBER_CURRENT_POINT,MEMBER_TOTAL_POINT,MEMBER_CREW_NUM,MEMBER_CREW_JOIN_DATE,MEMBER_LOCATION,MEMBER_ROLE\r\n"
         + "FROM MEMBER\r\n"
         + "WHERE MEMBER_CREW_NUM = ?";
   
   //랭킹높은순으로 정렬 관리자 FIXME 관리자 권한이 아닌사람들만
   private final String SERCHRANK="SELECT MEMBER_ID,MEMBER_PASSWORD,MEMBER_NAME,MEMBER_PHONE,MEMBER_PROFILE,MEMBER_REGISTRATION_DATE,MEMBER_CURRENT_POINT,MEMBER_TOTAL_POINT,MEMBER_CREW_NUM,MEMBER_CREW_JOIN_DATE,MEMBER_LOCATION,MEMBER_ROLE\r\n"
         + "FROM MEMBER\r\n"
   ...
   //관리자 권한 변경 ROLE, ID
   private final String UPDATEADMIN ="UPDATE MEMBER SET MEMBER_ROLE = '?' WHERE MEMBER_ID = '?'";
   
   //관리자가 아닌 신규회원 출력 (기간 7일)
   private final String ALLNEW ="SELECT MEMBER_ID,MEMBER_PASSWORD,MEMBER_NAME,MEMBER_PHONE,MEMBER_PROFILE,MEMBER_REGISTRATION_DATE,MEMBER_CURRENT_POINT,MEMBER_TOTAL_POINT,MEMBER_CREW_NUM,MEMBER_CREW_JOIN_DATE,MEMBER_LOCATION,MEMBER_ROLE\r\n"
         + "FROM MEMBER\r\n"
   ...
   public boolean update(MemberDTO memberDTO) {
      ...
      try {
         ...
         if(memberDTO.getMember_condition().equals("MEMBER_UPDATE")) {
         ...
         else if(memberDTO.getMember_condition().equals("MEMBER_UPDATECREW")) {
         ...
         else if(memberDTO.getMember_condition().equals("MEMBER_UPDATEADMIN")) {
   ...
   public MemberDTO selectOne(MemberDTO memberDTO) {
      ...
      try {
         //아이디로 검색 MEMBER_ID
         if(memberDTO.getMember_condition().equals("MEMBER_SERCHID")) {
            pstmt=conn.prepareStatement(SERCHID);
            pstmt.setString(1, memberDTO.getMember_id());
         }
         //아이디,비밀번호로 검색 MEMBER_ID, MEMBER_PASSWORD
         else if(memberDTO.getMember_condition().equals("MEMBER_SERCHIDPASS")) {
   ...
   public ArrayList<MemberDTO> selectAll(MemberDTO memberDTO) {
      ...
      try {
         //랭킹높은순으로 정렬
         if(memberDTO.getMember_condition().equals("MEMBER_SERCHRANK")) {
            pstmt=conn.prepareStatement(SERCHRANK);
         }
         //크루에 속한 회원목록 조회 MEMBER_CREW_NUM
         else if(memberDTO.getMember_condition().equals("MEMBER_SERCHCREW")) {
            pstmt=conn.prepareStatement(SERCHCREW);
            pstmt.setInt(1, memberDTO.getMember_crew_num());
         }
         //관리자가 아닌 신규회원 출력 (기간 7일)
         else if(memberDTO.getMember_condition().equals("MEMBER_ALLNEW")) {
            pstmt=conn.prepareStatement(ALLNEW);
         }
         else {
            System.err.println("condition 틀림");
            return null;
