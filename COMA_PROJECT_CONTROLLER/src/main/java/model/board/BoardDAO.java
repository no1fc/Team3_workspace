...
public class BoardDAO {
	...
	private final String ONE_SEARCH_TITLE_COUNT = "SELECT COUNT(*) AS BOARD_TOTAL FROM BOARD WHERE BOARD_TITLE LIKE '%'||?||'%'";
	...
	//작성자 ID로 검색 페이지네이션 윈도우함수 ROW_NUMBER()사용 BOARD_WRITER_ID, board_min_num, board_max_num
	private final String ALL_SEARCH_ID = "SELECT\r\n"
			+ "	BOARD_PAGENATION.RN,\r\n"
	...
	// 게시글 삭제 BOARD_NUM
	private final String DELETE ="DELETE FROM BOARD\r\n"
			+ "WHERE BOARD_NUM = ?"; 
	...
	private final String UPDATE_CONTENT_TITLE = "UPDATE BOARD SET BOARD_CONTENT = ?, BOARD_TITLE = ? WHERE BOARD_NUM = ?";
	...
	public boolean insert(BoardDTO boardDTO) {
		...
		try {
			...
			pstmt.setString(3,boardDTO.getBoard_location());
			pstmt.setString(4, boardDTO.getBoard_writer_id());
			int rs = pstmt.executeUpdate();
			if(rs<=0) {
				System.out.println("board.BoardDAO.insert 실패");
				return false;
			}
		}
	...
	public boolean update(BoardDTO boardDTO) {
		...
		try {
			...
			if(boardDTO.getBoard_condition().equals("BOARD_UPDATE_CONTENT_TITLE")) {
				pstmt = conn.prepareStatement(UPDATE_CONTENT_TITLE);
				pstmt.setString(1, boardDTO.getBoard_content());
				pstmt.setString(2, boardDTO.getBoard_title());
				pstmt.setInt(3, boardDTO.getBoard_num());
			}
			//게시글 조회수 변경 BOARD_CNT BOARD_NUM
			else if(boardDTO.getBoard_condition().equals("BOARD_UPDATE_CNT")) {
				pstmt = conn.prepareStatement(UPDATE_CNT);
				pstmt.setInt(1, boardDTO.getBoard_cnt());
				pstmt.setInt(2, boardDTO.getBoard_num());
			}
			else {
	...
	public boolean delete(BoardDTO boardDTO) {
		...
		try {
			...
			int rs = pstmt.executeUpdate();
			if(rs <= 0) {
				System.out.println("board.BoardDAO.delete 실패");
				return false;
			}
		}catch(SQLException e) {
	...
	public BoardDTO selectOne(BoardDTO boardDTO) {
		...
		try {
			//게시글 PK 검색 BOARD_NUM
			if(boardDTO.getBoard_condition().equals("BOARD_ONE")) {
				pstmt = conn.prepareStatement(ONE);
				pstmt.setInt(1, boardDTO.getBoard_num());
				sqlq="one";
			}
			// 전체 글 개수 FIXME
			else if(boardDTO.getBoard_condition().equals("BOARD_ONE_COUNT")) {
				pstmt = conn.prepareStatement(ONE_COUNT);
				sqlq = "count";
			}
			//ID로 검색한 글 개수 BOARD_WRITER_ID
			else if(boardDTO.getBoard_condition().equals("BOARD_ONE_SEARCH_ID_COUNT")) {
				pstmt = conn.prepareStatement(ONE_SEARCH_ID_COUNT);
				pstmt.setString(1, boardDTO.getBoard_searchKeyword());
				sqlq = "count";
			}
			//제목으로 검색한 글 개수 BOARD_TITLE
			else if(boardDTO.getBoard_condition().equals("BOARD_ONE_SEARCH_TITLE_COUNT")) {
				pstmt = conn.prepareStatement(ONE_SEARCH_TITLE_COUNT);
				pstmt.setString(1, boardDTO.getBoard_searchKeyword());
				sqlq = "count";
			}
			//이름으로 검색한 글 개수 MEMBER_NAME AS BOARD_WRITER_ID 
			else if(boardDTO.getBoard_condition().equals("BOARD_ONE_SEARCH_NAME_COUNT")) {
			...
			System.out.println("sqlq = "+sqlq);
	         ...
	         if (flag && sqlq.equals("one")) {
	            ...
	            System.out.println("BoardDAO.selectOne 검색 성공");
	            data = new BoardDTO();
	            data.setBoard_total(rs.getInt("BOARD_TOTAL"));
	         }
		}catch(SQLException e) {
	...
	public ArrayList<BoardDTO> selectAll(BoardDTO boardDTO){
		...
		try {
			//작성자 ID로 검색 페이지네이션 윈도우함수 ROW_NUMBER()사용 BOARD_WRITER_ID, board_min_num, board_max_num
			if(boardDTO.getBoard_condition().equals("BOARD_ALL_SEARCH_ID")) {
				pstmt = conn.prepareStatement(ALL_SEARCH_ID);
				pstmt.setString(1, boardDTO.getBoard_searchKeyword());
				pstmt.setInt(2, boardDTO.getBoard_min_num());
				pstmt.setInt(3, boardDTO.getBoard_max_num());
			}
			//제목으로 검색 페이지네이션 윈도우함수 ROW_NUMBER()사용 BOARD_TITLE, board_min_num, board_max_num
			else if(boardDTO.getBoard_condition().equals("BOARD_ALL_SEARCH_TITLE")) {
			...
			else if(boardDTO.getBoard_condition().equals("BOARD_ALL_SEARCH_NAME")) {
			...
			// 전체 글 출력(ALL) 페이지네이션 윈도우함수 ROW_NUMBER()사용 board_min_num, board_max_num
			else if(boardDTO.getBoard_condition().equals("BOARD_ALL")) {
				pstmt = conn.prepareStatement(ALL);
				pstmt.setInt(1, boardDTO.getBoard_min_num());
				pstmt.setInt(2, boardDTO.getBoard_max_num());
			}
			// 최신글5개 검색 
			else if(boardDTO.getBoard_condition().equals("BOARD_ALL_ROWNUM")) {
