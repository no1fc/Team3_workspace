...
public class BattleDAO {
	...
	private final String INSERT = "INSERT INTO COMA.BATTLE(BATTLE_GYM_NUM) VALUES (?)";
	...
	public boolean insert(BattleDTO battleDTO) {
		...
		try {
			...
			if(rs<=0) {
				System.err.println("com.coma.app.biz.battle.insert 실패");
				return false;
			}

		} catch (SQLException e) {
			System.out.println("com.coma.app.biz.battle.insert SQL문 실패");
			return false;
		}finally {
	...
	public boolean update(BattleDTO battleDTO) {
		...
		try {
			...
			int rs = pstmt.executeUpdate();
			if(rs<=0) {
				System.err.println("com.coma.app.biz.battle.update 실패");
				return false;
			}

		} catch (SQLException e) {
			System.err.println("com.coma.app.biz.battle.update SQL문 실패");
			return false;
		}finally {
			JDBCUtil.disconnect(pstmt,conn);
		}
	...
	private boolean delete(BattleDTO battleDTO) {
		System.err.println("com.coma.app.biz.battle.delete 시작");
		Connection conn=JDBCUtil.connect();
		PreparedStatement pstmt=null;
		try {
			...
			int rs = pstmt.executeUpdate();
			if(rs<=0) {
				System.err.println("com.coma.app.biz.battle.delete 실패");
				return false;
			}

		} catch (SQLException e) {
			System.err.println("com.coma.app.biz.battle.delete SQL문 실패");
			return false;
		}finally {
			JDBCUtil.disconnect(pstmt,conn);
		}
		System.out.println("com.coma.app.biz.battle.delete 성공");
		return true;
	}
	...
	public BattleDTO selectOne(BattleDTO battleDTO){
		...
		try {
			...
			if(flag && sqlq.equals("one")) {
				System.out.println("com.coma.app.biz.battle.selectOne 검색 성공");
				data = new BattleDTO();
				try {
			...
			System.err.println("com.coma.app.biz.battle.selectOne SQL문 실패");
			e.printStackTrace();
			return null;
		}finally {
			JDBCUtil.disconnect(pstmt,conn);
	...
	public ArrayList<BattleDTO> selectAll(BattleDTO battleDTO){
		System.out.println("com.coma.app.biz.battle.selectAll 시작");
		...
		try {
			...
			if(battleDTO.getModel_battle_condition().equals("BATTLE_ALL_ACTIVE")) {
				...
				pstmt.setInt(2, 10); //FIXME 변경된 코드입니다.
			}
			//해당 암벽장에서 실행된 크루전 전부 출력 BATTLE_GYM_NUM
			...
			System.err.println("com.coma.app.biz.battle.selectAll SQL문 실패");
			e.printStackTrace();
			return datas;
		}finally {
			JDBCUtil.disconnect(pstmt,conn);
		}
		System.out.println("com.coma.app.biz.battle.selectAll 성공");
		return datas;
